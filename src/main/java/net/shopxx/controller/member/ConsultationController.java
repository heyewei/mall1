...
public class ConsultationController extends BaseController {
	...
	public String list(Integer pageNumber, @CurrentUser Member currentUser, ModelMap model) {
		...
		model.addAttribute("page", consultationService.findPage(currentUser, null, null, pageable));
	...
	public ResponseEntity<?> list(Integer pageNumber, @CurrentUser Member currentUser) {
		Pageable pageable = new Pageable(pageNumber, PAGE_SIZE);
		return ResponseEntity.ok(consultationService.findPage(currentUser, null, null, pageable).getContent());
	}

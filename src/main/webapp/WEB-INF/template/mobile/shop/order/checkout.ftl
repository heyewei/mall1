						...
						[#if currentUser.balance > 0]
							<div id="useBalanceItem" class="[#if order.amountPayable <= 0 ]hidden-element [/#if]list-group-item">
								<div class="row">
									...
									<div class="col-xs-7">
										<span class="gray-darker">${currency(currentUser.balance, true)}</span>
									</div>
									<div class="col-xs-2 text-right">
					...
					<div class="list-group list-group-flat">
						[#if order.type == "general"]
							...
							[#if setting.isInvoiceEnabled && setting.isTaxPriceEnabled]
								<div class="list-group-item small">
									${message("Order.tax")}
									<strong class="pull-right red" data-name="tax" data-type="currency">${currency(order.tax, true)}</strong>
								</div>
							[/#if]
						[#elseif order.type == "exchange"]
							<div class="list-group-item small">
						...
						[#if order.isDelivery]
							<div class="list-group-item small">
								...
								<strong class="pull-right" data-name="freight" data-type="currency">${currency(order.freight, true)}</strong>
							</div>
						[/#if]
					</div>
				</div>

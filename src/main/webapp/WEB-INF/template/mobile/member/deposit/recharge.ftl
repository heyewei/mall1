	...
	<main>
		<div class="container-fluid">
			<form id="rechargeForm" action="${base}/payment" method="post">
				<input name="type" type="hidden" value="DEPOSIT_RECHARGE">
				...
				<div class="list-group list-group-flat">
					<div class="list-group-item">
						<div class="form-group">
							<label for="rechargeAmount">${message("member.deposit.rechargeAmount")}</label>
							<input id="rechargeAmount" name="rechargeAmount" class="form-control" type="text" maxlength="16" onpaste="return false;">
						</div>
					</div>
					<div id="feeItem" class="fee-item list-group-item small">
						${message("member.deposit.fee")}:
						<strong id="fee"></strong>
					</div>
					<div class="list-group-item small">${message("member.deposit.balance")}: ${currency(currentUser.balance, true)}</div>
				</div>
				[#if paymentPlugins??]
					<div class="panel panel-flat">
						<div class="panel-heading">${message("member.deposit.paymentPlugin")}</div>
